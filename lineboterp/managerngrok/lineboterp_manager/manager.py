...
def handle_message(event):
    ...
    msg = info.msg = event.message.text
    user_id = info.user_id = event.source.user_id
    user_state = info.user_state
    user_state1 = info.user_state1
    storage = info.storage
    global_Storage = info.global_Storage
    list_page = info.list_page
    duplicate_save = info.duplicate_save
    db = info.db
...
def checkdb():
    ...
    if minutes not in [0,15,30,45,60]:
        ...
        if minutes % 5 == 0:
            dbconnect1_job()
    # 執行年報表函式
    elif  modified_month == 1 and modified_day == 1 and modified_hour == 5 and modified_minutes == 0 :
        manager_month_report()
    # 執行月報表函式  
    elif  modified_day == 1 and modified_hour == 0 and modified_minutes == 0 :
        manager_year_report()
    else:
        dbconnect_job()
