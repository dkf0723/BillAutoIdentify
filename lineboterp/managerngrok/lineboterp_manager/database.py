...
def getmanuinf():
  id = info.user_id
  pid = info.storage[id + 'purchase_pid']
...
def orderdt():
  userid = info.user_id
  ordersearch = info.orderall[userid+'dt'] #本是使用者的ID
  query = f"""
          SELECT
...
def Product_status():
  ...
  pid = info.product[user_id + 'Product_Modification_Product_id']
