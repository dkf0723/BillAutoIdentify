...
def manager_manufacturers_list():
    ...
    if manufacturers_list == '找不到符合條件的資料。':
      ...
      pagemax = info.list_page[info.user_id+'廠商數量max']#9
...
def manager_products_manufacturers_list(manufacturer_id,choose): 
    ...
    if products_manufacturers_list == '找不到符合條件的資料。':
      ...
      if choose != 'stop':
        ...
        test1 = '【停售及截止商品 】'
        test2 = '【停售及截止商品列表 】'
      pagemin = info.list_page[info.user_id+page+'數量min']
      pagemax = info.list_page[info.user_id+page+'數量max']#9
...
def manager_categoryate_list(selected_category):
    ...
    if categoryate_list == '找不到符合條件的資料。':
      ...
      pagemin = info.list_page[info.user_id+'類別商品數量min']
      pagemax = info.list_page[info.user_id+'類別商品數量max']#9
